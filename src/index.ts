...
function run() {
  ...
  try {
    ...
    // Generate CSV Output
    const rows: string[] = [];
    rows.push(`Metric,Date,Value,Time Above ${ZERO_THRESHOLD} (min),Time Below ${ZERO_THRESHOLD} (min)`);
    ...
    for (const date of dates) {
        ...
        rows.push(`Daily Average Temp,${date},${dailyAvg.toFixed(2)},${timeAboveMin.toFixed(2)},${timeBelowMin.toFixed(2)}`);
